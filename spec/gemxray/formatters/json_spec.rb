# frozen_string_literal: true

RSpec.describe GemXray::Formatters::Json do
  describe "#render" do
    it "renders a report as pretty JSON" do
      result = GemXray::Result.new(
        gem_name: "foo",
        severity: :warning,
        reasons: [GemXray::Result::Reason.new(type: :unused, detail: "not used", severity: :warning)]
      )
      report = GemXray::Report.new(
        version: "0.1.0",
        ruby_version: "3.2.2",
        rails_version: "7.1.3",
        scanned_at: "2026-01-01T00:00:00+00:00",
        results: [result]
      )

      output = described_class.new.render(report)
      parsed = JSON.parse(output)

      expect(parsed["version"]).to eq("0.1.0")
      expect(parsed["ruby_version"]).to eq("3.2.2")
      expect(parsed["rails_version"]).to eq("7.1.3")
      expect(parsed["results"].length).to eq(1)
      expect(parsed["results"].first["gem_name"]).to eq("foo")
      expect(parsed["summary"]["total"]).to eq(1)
    end

    it "renders empty results" do
      report = GemXray::Report.new(
        version: "0.1.0",
        ruby_version: "3.2.2",
        rails_version: nil,
        scanned_at: "2026-01-01T00:00:00+00:00",
        results: []
      )

      output = described_class.new.render(report)
      parsed = JSON.parse(output)

      expect(parsed["results"]).to eq([])
      expect(parsed["summary"]["total"]).to eq(0)
    end

    it "produces valid JSON" do
      report = GemXray::Report.new(
        version: "0.1.0",
        ruby_version: "3.2.2",
        rails_version: nil,
        scanned_at: "2026-01-01T00:00:00+00:00",
        results: []
      )

      output = described_class.new.render(report)

      expect { JSON.parse(output) }.not_to raise_error
    end
  end
end
